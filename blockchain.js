...
function pagarTaxaDeRequerimento() {
  ...
  var additionalSettings = {
    value: ethers.utils.parseUnits(taxaDeRequerimento, 'wei')
  };
  contratoComSignatario
    ...
    .then((transacao) => {
      ...
      transacao
        .wait()
        .then((resultado) => {
          console.log("taxaDeRequerimento - o resultado foi ", resultado);
          if (resultado.status === 1) {
            $("#descricaoStatusTransacoes").html("Transação executada.");
          } else {
            $("#descricaoStatusTransacoes").html("Houve um erro na execução da transação no Ethereum.");
        ...
        .catch((err) => {
          console.error("taxaDeRequerimento - a transação foi minerada e houve um erro. Veja abaixo");
          console.error(err);
          $("#descricaoStatusTransacoes").html("Algo saiu errado: " + err.message);
        });
    })
      .catch((err) => {
        console.error("taxaDeRequerimento - tx só foi enviada");
        console.error(err);
        $("#descricaoStatusTransacoes").html("Algo saiu errado antes de enviar ao Ethereum: " + err.message);
    });
}
